(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.cartPrefix = "techturtle"; // Prefix string to be prepended to the cart's name in the session storage
			...
			if( this.$formCart.length ) {
				...
				if( items.length == 0 ) {
					...
					for( var i = 0; i < items.length; ++i ) {
					    ...
					    var html = "<div class='product'><div class='product-image'><img src='" + productImage + "'></div><div class='product-details'><div class='product-title'>" + product + "</div><div class='product-barcode'>" + barcode + "</div><div class='product-description'>" + productDescription + "</div></div><div class='product-price'>" + price + "</div><div class='product-quantity'><input type='number' value='" + qty + "' min='1'></div><div class='product-removal'><button type='button' class='remove-product'>Remove</button></div><div class='product-line-price'>" + totalValue + "</div></div>";
